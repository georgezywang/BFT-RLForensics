...
class SeparateLearner:
    ...
    def _train_critic(self, batch, rewards, terminated, mask):
        ...
        target_critic_outs = []
        target_critic_hidden = self.target_critic.init_hidden().unsqueeze(0).expand(batch.batch_size, -1)
        for t in range(batch.max_seq_length):
        ...
        critic_hidden = self.critic.init_hidden().unsqueeze(0).expand(batch.batch_size, -1)
